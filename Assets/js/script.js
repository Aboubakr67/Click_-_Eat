...
const toggleIngredient = (id, name) => {
  ...
  if (!isRemoved) {
    ...
    currentItem.removedIngredients = currentItem.removedIngredients.filter(
      (ing) => ing.id !== id
    );
  }
...
const toggleEntree = (id, name, price) => {
  ...
  if (!existingEntree) {
    ...
    // Remove entrée
    currentItem.entrees = currentItem.entrees.filter(
      (entree) => entree.id !== id
    );
  }
...
const toggleBoisson = (id, name, price) => {
  ...
  const existingBoisson = currentItem.boissons.find(
    (boisson) => boisson.id === id
  );
  ...
  if (!existingBoisson) {
    ...
    // Remove boisson
    currentItem.boissons = currentItem.boissons.filter(
      (boisson) => boisson.id !== id
    );
  }
