...
<div class="p-8">
    ...
    <div class="bg-white rounded-2xl p-8 shadow-sm mb-4 min-h-[500px]">
        <div class="flex justify-center items-center gap-8 h-full">
            ...
            <div onclick="setPaymentMethod('ESP')"
                class="bg-white rounded-2xl shadow-md p-8 w-64 h-64 flex flex-col items-center justify-center cursor-pointer hover:shadow-lg transition-shadow">
                <div class="w-24 h-24 mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-full h-full text-[#D84315]">
...
<script>
    ...
    function setPaymentMethod(method) {
        ...
        fetch('../Actions/PostCommande.php', {
            ...
            .then(response => {
                if (!response.ok) {
                    // Si la réponse n'est pas ok (statut HTTP 2xx), on jette une erreur
                    throw new Error(`Erreur HTTP: ${response.status}`);
                }
                // Vérifier si la réponse est bien en JSON
                const contentType = response.headers.get('Content-Type');
                if (contentType && contentType.includes('application/json')) {
                    return response.json(); // On parse la réponse en JSON si c'est du JSON
                } else {
            ...
            .then(data => {
                ...
                downloadBtn.textContent = 'Télécharger la facture (PDF)';
                downloadBtn.onclick = (e) => {
                    e.stopPropagation();
                    downloadAsPDF(invoiceContainer.querySelector('.bg-white'));
                };
